...
def dialogue_gen():
    ...
    composition_generator: Generator[tuple[Composition, str]]
    ...
    if configs.ARGS.separate_track_export:
        for (composition, file_suffix) in composition_generator:
            fix_and_write_mlt(XML(composition.xml()), file_suffix)
    else:
        raise ValueError('TODO')  # TODO


def process_components(components: list[str], lines: list[DialogueLine | SysLine]) -> Generator[tuple[Composition, str], None, None]:
...
def wrap_generate(gen_function: Callable[[list], Composition],
                  file_suffix: str, error_msg: str) -> Generator[tuple[Composition, str], None, None]:
...
def gen_all(lines: list[DialogueLine | SysLine]) -> Generator[tuple[Composition, str], None, None]:
...
def gen_text(lines: list[DialogueLine | SysLine]) -> Generator[tuple[Composition, str], None, None]:
    print("Generating text component")
    yield from wrap_generate(lambda: text_gen.generate(lines), 'text',
                             'Error while generating text:')


def gen_header(lines: list[DialogueLine | SysLine]) -> Generator[tuple[Composition, str], None, None]:
...
def gen_chars(lines: list[DialogueLine | SysLine]) -> Generator[tuple[Composition, str], None, None]:
...
def gen_char(lines: list[DialogueLine | SysLine], character: str) -> Generator[tuple[Composition, str], None, None]:
